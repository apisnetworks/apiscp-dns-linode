<?php declare(strict_types=1);
	...
	class Module extends \Dns_Module implements ProviderInterface
	{
		...
		//
		// Workaround for non-standard SRV usage
		// Linode transforms service.proto into service.<proto literal>.proto
		// e.g. _sip._tcp becomes _sip.tcp._tcp
		public function record_exists(
			...
			if (strtoupper($rr) === 'SRV') {
				// SRV presently broken
				$record = new Record($zone, ['name' => $subdomain, 'rr' => $rr, 'parameter' => $parameter]);
				return null !== $this->getRecordId($record);
			}
			return parent::record_exists($zone, $subdomain, $rr, $parameter); // TODO: Change the autogenerated stub
		}


		protected function addCache(BaseRecord $r): void
		{
			// Linode API is presently broken on SRV records
			// _sip._tcp stored as _sip.tcp._tcp
			if ($r->matches('rr', 'SRV')) {
				$r['name'] = preg_replace('/^(_[^\.])\._([^.]+)/', '\1.\2._\2', $r['name']);
			}

			parent::addCache($r); // TODO: Change the autogenerated stub
		}
		...
		protected function getCacheKey(\Opcenter\Dns\Record $r): string
		{
			...
			return parent::getCacheKey($r); // TODO: Change the autogenerated stub
		}
